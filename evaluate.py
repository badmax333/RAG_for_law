...
class OpenAIJudge:
    ...
    def __init__(self, model_name: str | None = None, timeout_sec: float = 60.0):
    ...
    def _context_to_text(self, context: List[Any], limit_chars: int = 6000) -> str:
        """
        Converts retrieved context into readable text.
        Supports: list[str], list[dict], langchain Documents-ish objects.
        """
        lines = []
        for c in context or []:
            if isinstance(c, str):
                ...
                # common formats: {"text": "...", "source": "..."} or {"page_content": "...", "source": "..."}
                src = str(c.get("source", "") or "")
                text = str(c.get("text") or c.get("page_content") or c.get("content") or "")
            else:
                # try to handle objects with attributes
                src = getattr(c, "metadata", {}).get("source", "") if hasattr(c, "metadata") else ""
                text = getattr(c, "page_content", "") if hasattr(c, "page_content") else str(c)
    ...
    def evaluate(
        ...
        system = (
            "Ты — строгий эксперт-оценщик качества ответов RAG-системы по ПДД РФ. "
            "Оценивай честно и консервативно. "
...
Контекст (документы, найденные ретривером):
{context_str}
...
Оцени по шкале 1..10:
- relevance
- faithfulness (не выдумано ли; основано ли на контексте)
- completeness
- clarity
...
{{
  ...
  "completeness": 7,
  "clarity": 8,
  "justification": "Короткое обоснование (1-3 предложения)."
}}
"""
        ...
        try:
            ...
            # если модель вдруг добавила лишнее — вытащим JSON
            ...
            return {
                ...
                "completeness": 0.0,
                "clarity": 0.0,
                "justification": f"Judge failed: {e}",
            }
...
def main():
    ...
    if args.use_llm_judge:
        # нужен API ChatGPT .env!
        judge = OpenAIJudge()
        # тут скачивать на локалку мистраль 
        # judge = LLMJudge() 
